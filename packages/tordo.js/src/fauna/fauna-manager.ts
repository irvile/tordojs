...
export class FaunaManager {
  ...
  private getFaunaRegion() {
    return process.env.FAUNADB_REGION === 'undefined' || process.env.FAUNADB_REGION === undefined
      ? 'classic'
      : process.env.FAUNADB_REGION!
  }

  private isTordoLocal() {
    return process.env.TORDO_FAUNADB_LOCAL === 'undefined' || process.env.TORDO_FAUNADB_LOCAL === undefined
      ? true
