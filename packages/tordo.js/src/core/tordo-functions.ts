...
import { q, ExprVal as ExprValue, faunadb } from '../fauna'
...
export async function createTestDatabase(specName: string): Promise<any> {
  ...
  const database: any = await faunaClient.query(
    Do(If(Exists(Database(specName)), Delete(Database(specName)), false), CreateDatabase({ name: specName }))
  )

  const adminKey: any = await faunaClient.query(CreateKey({ database: database.ref, role: 'admin' }))
...
export async function deleteAllDocuments() {
  ...
  const promises = collections.data.map((collectionReference) => {
    const deleteAllDocumentsFQL = If(
      Exists(q.Collection(collectionReference.id)),
      q.Map(q.Paginate(q.Documents(q.Collection(collectionReference.id))), q.Lambda('ref', Delete(q.Var('ref')))),
      true
    )
...
export async function createCollection(collectionName: string) {
  const faunaClient = getFaunaClient()
  return faunaClient.query(If(Exists(Collection(collectionName)), true, CreateCollection({ name: collectionName })))
}

export async function deleteCollection(collectionName: string) {
  const faunaClient = getFaunaClient()
  return faunaClient.query(If(Exists(Collection(collectionName)), Delete(Collection(collectionName)), true))
}
...
export async function createIndex(
  ...
  let indexTerms: ExprValue[] | undefined
  let indexValues: ExprValue[] | undefined
...
export const listCollections = async (): Promise<Array<string>> => {
  ...
  return collections.data.map((collectionReference) => collectionReference.id)
}
