...
import { q, ExprArg as ExprArgument } from '../fauna'
import {
  FaunaDocument,
  FaunaReferenceObject,
  IndexOptions,
...
export class BaseCollection {
  ...
  public static async create<T extends BaseCollection>(
  ...
  ): Promise<ObjectPropertiesOptional<T> & FaunaReferenceObject> {
    ...
    const documentReference: FaunaDocument = await faunaClient.query(
      q.Create(q.Collection(_this.getCollectionName()), { data: properties })
    )

    return formatter(documentReference)
  }
  ...
  public static async findMany() {
    ...
    documents.sort((documentLeft, documentRight) => documentRight.ref.id - documentLeft.ref.id)
  ...
  static async createIndexes() {
    for (const [indexName, indexOptions] of this.getIndexesMap().entries()) {
      await createIndex(
        this.getCollectionName(),
        indexName,
        indexOptions.isUnique,
  ...
  static async find<T extends BaseCollection>(
    ...
    const indexMatches: ExprArgument[] = []
    const _this = <typeof BaseCollection>this
    ...
    try {
      const faunaClient = getFaunaClient()
      return await faunaClient.query(q.Map(q.Paginate(q.Union(indexMatches)), q.Lambda('doc', q.Get(q.Var('doc')))))
    } catch (error) {
  ...
  public static async update<T extends BaseCollection>(
    ...
    try {
      return await faunaClient.query(q.Update(q.Ref(q.Collection(collectionName), id), { data: properties }))
    } catch (error) {
  ...
  public static async findBetweenDates(columnDate: string, start: string, end: string) {
    try {
      const faunaClient = getFaunaClient()
      const documents: any = await faunaClient.query(
        q.Map(
